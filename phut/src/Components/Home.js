...
const Home = () => {
  ...
  const setCompleted = async (_id) => {
    ...
    try {
      const data = await fetch(
        `http://localhost:5001/api/class/isComplete/${_id}`,
        {
          method: 'PATCH',
          headers: {
  ...
  const add = async (name) => {
    ...
    try {
      const data = await fetch('http://localhost:5001/api/class', {
        method: 'Post',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const edit = async (name, _id) => {
    try {
      const data = await fetch(`http://localhost:5001/api/class/${_id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
